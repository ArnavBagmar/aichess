...
python -m tools.label_positions `
  ...
  --positions 100000 --depth 10 --max-abs-score 1000
```

The generator uses one teacher thread, fixed-seed legal randomized games, score filtering, and
writes a SHA-256 provenance manifest. The JSONL dataset and its machine-specific manifest are
...
starts from the current handcrafted evaluation and learns only a residual. A deterministic 10%
